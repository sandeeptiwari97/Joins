...
public class SortOperator  {
  ...
  public Run sortRun(Run run) throws DatabaseException {
    ...
    while (runIterator.hasNext()) {
      runList.add(runIterator.next());
    }
    Collections.sort(runList, comparator);
    sortedRun.addRecords(runList);
    return sortedRun;
  }
  ...
  public Run mergeSortedRuns(List<Run> runs) throws DatabaseException {
    ...
    PriorityQueue<Pair<Record, Integer>> runPQ = new PriorityQueue<Pair<Record, Integer>>(new RecordPairComparator());
    ...
    for (int i = 0; i < runIterators.size(); i++) {
      runPQ.add(new Pair<Record, Integer>(runIterators.get(i).next(), i));
    }

    Pair<Record, Integer> lowPair = runPQ.poll();
    while (lowPair != null) {
      Record r = lowPair.getFirst();
      int i = lowPair.getSecond();
  ...
  public List<Run> mergePass(List<Run> runs) throws DatabaseException {
    ...
    while (runIterator.hasNext()) {
      List<Run> buffRuns = new ArrayList<>();
      for (int i = 0; i < numBuffers - 1; i++) {
        if (runIterator.hasNext()) {
          buffRuns.add(runIterator.next());
        }
      }
      merged = mergeSortedRuns(buffRuns);
  ...
  public String sort() throws DatabaseException {
    ...
    while (pageIter.hasNext()) {
      Run run = new Run();
      BacktrackingIterator<Record> recIter = this.transaction.getBlockIterator(this.tableName, pageIter, numBuffers);
      while (recIter.hasNext()) {
        records.add(recIter.next());
      }
      run.addRecords(records);
      Run sortRun = sortRun(run);
      runs.add(sortRun);
    }
    while (runs.size() > 1) {
      runs = mergePass(runs);
