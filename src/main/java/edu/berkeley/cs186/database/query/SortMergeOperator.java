...
public class SortMergeOperator extends JoinOperator {
  ...
  private class SortMergeIterator extends JoinIterator {
    ...
    */
     ...
     private boolean allJoinFound;

     private LR_RecordComparator lrCompare = new LR_RecordComparator();

     private SortOperator leftSO;
     private SortOperator rightSO;
    ...
    public SortMergeIterator() throws QueryPlanException, DatabaseException {
      ...
      leftSO = new SortOperator(getTransaction(), getLeftTableName(), new LeftRecordComparator());
      rightSO = new SortOperator(getTransaction(), getRightTableName(), new RightRecordComparator());

      leftTableName = leftSO.sort();
      rightTableName = rightSO.sort();
    ...
    private void fetchNextRecord() throws DatabaseException {
        nextRecord = null;
        while (!allJoinFound) {
            while (lrCompare.compare(leftRecord, rightRecord) < 0 && !allJoinFound) {
            ...
            while (lrCompare.compare(leftRecord, rightRecord) == 0) {
              while (lrCompare.compare(leftRecord, rightRecord) == 0) {
                if (hasNext()) {
                  return;
                }
