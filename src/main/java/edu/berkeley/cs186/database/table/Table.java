...
public class Table implements Iterable<Record>, Closeable {
  ...
  public class RIDPageIterator implements BacktrackingIterator<RecordId> {
    ...
    private Page page;
    private byte[] bitmap;
    private int prevMarkedPosition;
    private int ithBit;
    private int lastOne;
    public RecordId markedPrevRecordId = null;
    ...
    public RIDPageIterator(Page page) {
      this.page = page;
      bitmap = getBitMap(page);
      ithBit = 0;
      lastOne = 0;
      prevMarkedPosition = -1;
//      throw new UnsupportedOperationException("hw3: TODO");
    }

    public boolean hasNext() {
      Bits.Bit bit;
      if (ithBit < bitmap.length*8) { //numRecordsPerPage
        bit = Bits.getBit(bitmap, ithBit);
      } else {
        return false;
      }
    ...
    public RecordId next() {
      if (hasNext()) {
        prevRecordId = new RecordId(page.getPageNum(), (short) ithBit);
        ithBit += 1;
        return prevRecordId;
      }
    ...
    public void mark() {
      ...
      prevMarkedPosition = lastOne;//ithBit;
      markedPrevRecordId = prevRecordId;
    }
    ...
    public void reset() {
      ...
      ithBit = prevMarkedPosition;
      prevRecordId = markedPrevRecordId;
    }
  }
...
////        page = block.next();
//        blockIter = new RIDPageIterator(block.next());
////        blockIter = new RIDPageIterator(page);
